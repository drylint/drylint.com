...
查找并返回`targetObj`对象的`PropKey`属性，如果没有该属性，则返回`undefined`。
...
将`target`对象的`name`属性赋值为`value`

> 如果传入第四个参数 `receiver`，则整个`set`操作都是针对 `receiver` 对象。
...
注意，如果 `Proxy` 对象和 `Reflect` 对象联合使用，前者拦截赋值操作，后者完成赋值的默认行为。如果`Reflect.set()`传入了第四个参数`receiver`，那么`Reflect.set`会触发`Proxy.defineProperty`拦截。
...
#### (5) `Reflect.construct(targetFunc, args)` targetFunc 必须是函数
...
如果`Reflect.defineProperty`的第一个参数不是对象，就会抛出错误，比如`Reflect.defineProperty(1, 'foo')` 会报错。
